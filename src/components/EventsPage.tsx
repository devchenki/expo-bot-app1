...
export function EventsPage() {
  ...
  const { events: apiEvents, loading, completeEvent, deleteEvent } = useEvents();
  ...
  const formatEvent = (apiEvent: ApiEvent): Event => {
    ...
    const laptops = typeof equipment.laptops === "number" ? equipment.laptops : (Array.isArray(equipment.laptops) ? equipment.laptops.length : 0);
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Мероприятие "{eventToDelete?.name}" будет удалено безвозвратно. 
              Все установки этого мероприятия будут сохранены.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
